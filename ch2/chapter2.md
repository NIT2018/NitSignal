...
- C)	卷积求和的几个步骤：
- a)	
- D)	课堂练习，待定，矩形窗卷积，一个是相同宽度，一个是不同宽度。y(t)=$ \lim_{δ \to 0} \sum_{k=-\infty}^{+\infty} x(kδ)\hat{h}_{kΔ}(t)Δ $
## §2.3　连续时间LTI系统的时域分析
- 1)	根据前面式子，我们有：x(t)=$ \lim_{δ \to 0} \sum_{k=-\infty}^{+\infty} x(kδ)δ_δ(t-kδ)δ$ 
- A)	同样道理，如果我们知道$ δ_δ(t-kδ) $的响应是$ \hat{h}_kδ(t) $，根据叠加性质，系统输出为y(t)=$ \lim_{δ \to 0} \sum_{k=-\infty}^{+\infty} x(kδ)δ_δ(t-kδ)δ$，这个结果跟离散信号类似。
