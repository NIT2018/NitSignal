...
$$x_r(t)={\it x_p(t)*h(t)}=(\sum_{n=-\infty}^{+\infty} x(nT) \theta (t-nT)* \left( \frac{T}{t\pi}sin(\omega_ct) \right) ={\it \sum_{n=-\infty}^{+\infty} {\it x(nT)\frac {T}{\pi(t-nT)}}}sin(\it \omega_c(t-nT))$$
