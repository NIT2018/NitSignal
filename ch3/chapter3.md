...
A)	解：  $X(j\omega)=\int_{\-infty}^{\+infty} x(t){\it e}^{-jkwt},t=\int_{-T_1}^{T_1} {\it e}^{-jkwt},t=2\frac{\sin\omegaT_1}{\omega}$
B)	如图所示（奥本海默P146图4.17）
5)	例5：已知信号频谱X（jw）=$ \begin{cases} 1, |\omega|<T_1 \\  0,|\omega|>T_1 \end{cases}$，求原始信号， 
A)	解，利用傅立叶反变换公式，得 
