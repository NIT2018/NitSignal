...
2)	缩放的话，跟连续信号有所不同；
    ...
    D)	对应的傅立叶变换为 $$ X_{(k)}({\it e}^{j\omega})=\sum_{n=-\infty}^{+\infty} x_{(k)}[n]{\it e}^{-j\omega n}=\sum_{r=-\infty}^{+\infty} x_{(k)}[rk]{\it e}^{-j\omega rk}=\sum_{r=-\infty}^{+\infty} x_{(k)}[r]{\it e}^{-j(k\omega) n}=X({\it e}^{jk\omega})$$
