...
### 4.4.2 周期信号的傅立叶变换 
1)	考查上例2和傅立叶反变换公式$$x[n]=\frac{1}{2\pi}\int_{2\pi} X()$$ ，若令 ，则在任意一个2π区间内对 利用上面反变换公式，可得 ，所以周期信号x[n]的傅立叶变换为上式所代表的信号，如下图所示。(奥本海默第一版P243图5.16)$$\int_0^{T_0} {x(t){\it e}^{-jn\omega_0 t}} \,{\rm d}t$$
