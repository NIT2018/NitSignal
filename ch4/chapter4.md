...
2)	例2：求信号x[n]的频谱。$$x[n]=a_{|n|},|a|<1 $$
    ...
    a)	解： $$X({\it e}^{j\omega})=\sum_{n=-\infty}^{+\infty} x[n]{\it e}^{-j\omega n}=\sum_{n=0}^{=\infty} a^n{\it e}^{-j\omega n}+\sum_{n=-\infty}^{-1} a^{-n}{\it e}^{-j\omega n}=\sum_{n=0}^{+\infty} (a{\it e}^{-j\omega})^n+\sum_{n=-\infty}^{0} (a{\it e}^{j\omega})^{-n}-1=$$
