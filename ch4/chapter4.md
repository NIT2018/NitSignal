...
2)	证明：$$\sum_{n=-\infty}^{+\infty} {|x[n]|}^2=\sum_{n=-\infty}^{+\infty} x[n] \cdot x^*[n]=\sum_{n=-\infty}^{+\infty}  \brace x[n](\frac{1}{2\pi}\int_{2\pi} X({\it e}^{j\omega}) {\it e}^{j\omega n} \ {\rm d}\omega) \rbrace$$ 
3)	 
4)	交换积分和求和顺序，得
